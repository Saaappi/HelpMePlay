...
events:RegisterEvent("ADVENTURE_MAP_OPEN")
events:SetScript("OnEvent", function(self, event, ...)
	if ( event == "ADVENTURE_MAP_OPEN" ) then
		...
		if ( mapID == 1011 ) then -- Zandalar (Alliance: Footholds | Horde: Leveling)
		...
		if ( mapID == 1014 ) then -- Kul Tiras (Alliance: Leveling | Horde: Footholds)
			if ( faction == "Alliance" ) then
				if HelpMePlayDB.ZoneId_BFA[faction] == 0 or HelpMePlayDB.ZoneId_BFA[faction] == false or HelpMePlayDB.ZoneId_BFA[faction] == nil then return false end
		...
		if ( mapID == 2057 ) then -- Dragonflight
