	...
	if event == "PLAYER_LEVEL_CHANGED" then
		...
		if newLevel % 5 == 0 then
			...
			if GetNumUnspentTalents() > 0 then
				...
				addonTable.Print(L_GLOBALSTRINGS["Colored Addon Name"] .. ": " .. string.format(L_GLOBALSTRINGS["Level Congrats Text"], newLevel, GetSpellLink(addonTable.TALENTS[classId][specId][newLevel])))
			end
		end
	end
end)